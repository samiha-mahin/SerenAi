...
interface SessionContextType {
  user: User | null;
  loading: boolean; // Indicates if the session is being checked
...
//Creates a global box to store session information
// At start the box is empty (undefined) . Later we will fill it with data by using a Provider

export function SessionProvider({ children }: { children: React.ReactNode }) {
  const [user, setUser] = useState<User | null>(null); // Stores user data
  ...
  const checkSession = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const { password, ...safeUserData } = userData; //Take password out of userData, and put everything else into safeUserData.
